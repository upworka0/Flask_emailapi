...
class MailMessage(object):
    ...
    def get_message(self):
        ...
        part1 = MIMEMultipart("related")
        part1.attach(MIMEText(self.template.render(), 'html'))
        ...
        for file in self.templatefiles:
            ...
            # msgImage['Content-Disposition']='inline'
            part1.attach(msgImage)
...
def send(mail_msg, mail_server=MailServer()):
    ...
    text_file = open(path + "/test.txt", "w")
    text_file.write("email should be sent")
    text_file.close()
...
@app.route('/')
def hello_world():
    return render_template('index.html_')
...
def EmailSendingUint(prclim, Reply_to, Email_To, templateID_folder, templateID, address, price, name, subject,
    ...
    text_file = open(path + "/test2.txt", "w")
    text_file.write("emailsendingunit started")
    text_file.close()
    ...
    femail = 'frontdesk@la-retrofit.com'
    femailp = 'MehrdadMehran1@'
